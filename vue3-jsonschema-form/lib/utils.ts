...
export function resolveSchema(schema: Schema, rootSchema = {}, formData = {}) {
  /**
   * "$ref": "#/doc/xxx"  ->  (路径) 是在完整的 JSONSchema 上定义的
   *
   * @schema 传进来的是具体的某个节点的 schema
   * @rootSchema  是完整的 JSONSchema
   */
  if (hasOwnProperty(schema, '$ref')) {
     ...
     * }
     *
     * 在某个表单项里面选择了某个结果之后，下面的表单会进行相应的变动
     */
...
/**
 * retrieveSchema 会遍历我们传入的 JSONSchema 对象  这个对象会返回我们希望得到的一个新的 JSONSchema
 */
...
export function getMatchingOption(
  ...
  for (let i = 0; i < options.length; i++) {
    ...
    if (option.properties) {
      ...
      // Remove the "required" filed as it's likely that not all fileds have
